		  	...
		  	<div class="row">
			    ...
			    <form method="post">
				  <div class="form-group">
				   ...
				   <div class="form-group">
				    <label for="alt">ID Alternatif</label>
				    <select class="form-control" id="alt" name="alt">
				    	<option value='<?php echo $eks->alt ?>'><?php echo $eks->alt ?></option>
				    	<?php
						$stmt3 = $alte->readAll();
						while ($row3 = $stmt3->fetch(PDO::FETCH_ASSOC)){
							extract($row3);
							echo "<option value='{$id_alternatif}'>{$nama_alternatif} ({$id_alternatif})</option>";
						}
					    ?>
				    </select>
				  </div>
